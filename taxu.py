...
@bot.on_message(filters.text == 'code')
...
def send_dice_v1(chat_id):
  response = requests.get(
      f'https://api.telegram.org/bot{bot_token}/sendDice?chat_id={chat_id}')
...
@bot.on_message(filters.command('chuyentien'))
...
@bot.on_message(filters.command("ctien"))
def set_balance(msg):
...
@bot.on_message(filters.text & filters.user(user_state)
...
@bot.on_message(ffilters.text & filters.user(user_state)
...
@bot.on_message(filters.text & filters.user(user_state)
                     and user_state[Message.from_user.id] == "baucua_bet_amount")
...
@bot.on_message(filters.text & filters.user(user_state)
                     and user_state[Message.from_user.id][0] == "baucua_bet_item")
...
def send_dice_v2(chat_id):
  response = requests.get(
      f'https://api.telegram.org/bot{bot_token}/sendDice?chat_id={chat_id}&emoji=🎰'
  )
  if response.status_code == 200:
    data = response.json()
    if 'result' in data and 'dice' in data['result']:
...
#hàm xử lý game slot
@bot.callback_query_handler(filters.text == "game_slot")
...
@bot.callback_query_handler(filters.text_filter in ["momo", "bank"])
...
@bot.callback_query_handler(filters.text == ("game_"))
...
@bot.on_message(filters.command('xoso'))
...
@bot.on_message(filters.text in ["Lô 2 Số", "Lô 3 Số", "Lô 4 Số", "Xiên 2", "Xiên 3", "Xiên 4", "Đề 2 Số", "Đề 3 Số", "Đề 4 Số", "Đầu", "Đuôi"])
